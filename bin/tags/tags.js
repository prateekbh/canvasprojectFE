...
riot.tag2('gp-editimg', '<canvas id="drawer"></canvas><canvas id="bgcanvas"></canvas><button class="save"><svg xmlns="http://www.w3.org/2000/svg" fill="#FFFFFF" height="24" viewbox="0 0 24 24" width="24"><path d="M0 0h24v24H0z" fill="none"></path><path d="M17 3H5c-1.11 0-2 .9-2 2v14c0 1.1.89 2 2 2h14c1.1 0 2-.9 2-2V7l-4-4zm-5 16c-1.66 0-3-1.34-3-3s1.34-3 3-3 3 1.34 3 3-1.34 3-3 3zm3-10H5V5h10v4z"></path></svg></button><button class="pr" onclick="{raisepr}"><svg xmlns="http://www.w3.org/2000/svg" fill="#FFFFFF" height="24" viewbox="0 0 24 24" width="24"><path d="M0 0h24v24H0z" fill="none"></path><path d="M19.35 10.04C18.67 6.59 15.64 4 12 4 9.11 4 6.6 5.64 5.35 8.04 2.34 8.36 0 10.91 0 14c0 3.31 2.69 6 6 6h13c2.76 0 5-2.24 5-5 0-2.64-2.05-4.78-4.65-4.96zM14 13v4h-4v-4H7l5-5 5 5h-3z"></path></svg></button>', '', '', function(opts) {
	var curr_img=null;
	this.on("mount",function(){
		document.querySelector('body').style.overflow="hidden";
		var el = document.getElementById('drawer');
...
riot.tag2('gp-home', '<gp-userpic each="{item,i in data}" data="{item}"></gp-userpic>', '', '', function(opts) {
		var self=this;
		this.data=[];
		this.on("mount",function(){
...
riot.tag2('gp-profile', '<div class="loader {profileData ? \'hidden\' : \'\'}"><gp-loader></gp-loader></div><div class="profile {!profileData ? \'hidden\' : \'\'}"><div class="userinfo"><div class="pic"><img class="userpic" riot-src="{profileData.user.avatar_url}"></img></div><div class="info"><div class="name">{profileData.user.name}</div><div class="desc">front end engineer @ nowhere</div><div class="container-followers"><a class="followers" href="/followers/prateek">{profileData.user.followers.length} followers</a></div></div><button if="{profileData.user.account_id!==window.uid}" class="btn-follow" onclick="{follow}"> {isFollowing||profileData.is_follower?\'FOLLOWING\':\'FOLLOW\'} </button></div><div class="tabs"><div class="tab own selected" onclick="{selTab}"><div class="num">{profileData.owned_images&&profileData.owned_images.length}</div><div class="label">OWN</div></div><div class="tab cloned" onclick="{selTab}"><div class="num">{profileData.cloned_images&&profileData.cloned_images.length}</div><div class="label">CLONED</div></div><div class="tab pr" if="{profileData.user.account_id===window.uid}" onclick="{selTab}"><div class="num">{profileData.pull_requests&&profileData.pull_requests.length}</div><div class="label">PRs</div></div></div><div class="content"><div if="{selectedTab===0}"><a class="imagelink" each="{image in profileData.owned_images}"><img class="imgthumb" riot-src="{window.ip+\'/image/\'+ image.image_id +\'/data\'}"></img></a></div><div if="{selectedTab===1}"><a class="imagelink" href="/img/edit/{image.image_id}/{image.source_image_id}" each="{image in profileData.cloned_images}"><img class="imgthumb" riot-src="{window.ip+\'/image/\'+ image.image_id +\'/data\'}"></img></a></div><div if="{selectedTab===2}"><a class="pr" each="{pr in profileData.pull_requests}" href="/pr/{pr.pull_request_id}"><img class="sender" riot-src="{pr.sender.avatar_url}"></img><span class="info"><b>{pr.sender.name}</b> sent a pr on</span><img class="source" riot-src="{window.ip + ⁗/image/⁗+pr.original_image.image_id+⁗/data⁗}"></img></a></div></div></div>', '', '', function(opts) {
		var self=this;
		this.selectedTab=0;
...
riot.tag2('gp-pullrequest', '<canvas id="drawer"></canvas><canvas id="bgcanvas"></canvas><button class="accept" onclick="{acceptpr}"><svg xmlns="http://www.w3.org/2000/svg" fill="#FFFFFF" height="24" viewbox="0 0 24 24" width="24"><path d="M0 0h24v24H0z" fill="none"></path><path d="M9 16.2L4.8 12l-1.4 1.4L9 19 21 7l-1.4-1.4L9 16.2z"></path></svg></button><button class="pr" onclick="{rejectpr}"><svg xmlns="http://www.w3.org/2000/svg" fill="#FFFFFF" height="24" viewbox="0 0 24 24" width="24"><path d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z"></path><path d="M0 0h24v24H0z" fill="none"></path></svg></button>', '', '', function(opts) {
	var curr_img=null;
	this.on("mount",function(){
		document.querySelector('body').style.overflow="hidden";
		var el = document.getElementById('drawer');
...
riot.tag2('gp-userpic', '<div class="userinfo"><div class="piccontainer"><img class="userpic" height="50" width="50" riot-src="{this.opts.data.user_avatar_url}"></img></div><a href="/user/{this.opts.data.account_id}" class="username">{this.opts.data.user_name}</a><span class="time">15m</span></div><div class="painting" onclick="{toggleCopy}"><img class="img-painting" riot-src="{window.ip+⁗/image/⁗+this.opts.data.image_id+⁗/data⁗}"><button if="{!this.opts.data.is_cloned}" class="copy hidden" onclick="{cloneImage}"><svg xmlns="http://www.w3.org/2000/svg" fill="#FFFFFF" height="24" viewbox="0 0 24 24" width="24"><path d="M0 0h24v24H0z" fill="none"></path><path d="M16 1H4c-1.1 0-2 .9-2 2v14h2V3h12V1zm3 4H8c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h11c1.1 0 2-.9 2-2V7c0-1.1-.9-2-2-2zm0 16H8V7h11v14z"></path></svg></button></div>', '', '', function(opts) {
