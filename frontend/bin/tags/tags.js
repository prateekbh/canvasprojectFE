...
riot.tag2('gp-login', '<div class="title titlefont"> KrowdKanvas </div><div class="fblogin" if="{!userProfile}"><img if="{!loading}" src="https://www.takingwings.in/wp-content/plugins/wordpress-social-login/assets/img/32x32/wpzoom/facebook.png" width="200" onclick="{startLoading}"><material-spinner if="{loading}"></material-spinner></div>', '', '', function(opts) {
		var self=this;
		var userStore=veronica.flux.Stores.getStore("UserStore");
...
riot.tag2('gp-profile', '<div class="cover" riot-style="background-image:url(\'{userProfile.picture.data.url}\')"></div><div class="userinfo"><img class="userpic" riot-src="{userBigPic}" height="100" width="100"></img><div class="userdetails"><div class="username">Prateek Bhatnagar</div><div class="followers"><span class="count">22</span> followers</div></div></div>', '', '', function(opts) {
		var self = this;
		var userStore = veronica.flux.Stores.getStore("UserStore");
