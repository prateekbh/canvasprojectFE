...
riot.tag2('gp-profile', '<div class="profilepic {userPic?\'loaded\':\'\'}" riot-style="{userPic?\'background-image:url(\\\'\'+userPic+\'\\\')\':\'\'}"><div class="username">{userProfile.name}</div><gp-followbutton following="{true}" if="{userProfile&&ownerProfile&&ownerProfile.user.profile.id!==userProfile.user.profile.id}"></gp-followbutton></div><div class="usercontent" if="{userProfile}"><material-tabs useline="true" tabs="{tabs}" __selected="{selectedTab}" tabchanged="{tabChanged}"></material-tabs><div class="tabcontent tab{selectedTab}" onswipeleft="{incTabsIndex}" onswiperight="{decTabsIndex}"><div class="tab tab-owned"><div class="ownedcontainer"><img each="{pic, index in userProfile.owned_images}" height="{(window.innerWidth/3)-2}" class="ownedpic" riot-src="{pic.url}"></img></div></div><div class="tab tab-contri"> Tab2 </div></div></div><div class="usercontent" if="{!userProfile}"><div class="loader"><material-spinner></material-spinner></div></div>', '', '', function (opts) {
  var _this2 = this;
...
function UserAction() {
  ...
  this.fetchUserProfile = function (uid, sid) {
    ...
    fetch(apiBase + "/user/profile/details/" + uid, {
      ...
      _this3.Dispatcher.trigger("user:fetchprofile:success", data);
    }).catch(function (e) {
      _this3.Dispatcher.trigger("user:fetchprofile:failed", e);
    });
  };
  ...
  this.loginUser = function (fbprofile, oauth) {
    ...
    fetch(window.apiBase + "/user/signIn", {
      ...
      _this4.Dispatcher.trigger("user:login:success", data);
    }).catch(function (e) {
      _this4.Dispatcher.trigger("user:login:failure", { data: e });
    });
  };
...
function ImageActions() {
  this.saveImage = function (img, tags, description, sessionId) {
    ...
    fetch(window.apiBase + "/image/save", {
      ...
      _this5.Dispatcher.trigger("img:save:success", data);
    }).catch(function (e) {
      _this5.Dispatcher.trigger("img:save:failed", {});
    });
  };
}
...
function ImageStore() {
  ...
  this.Dispatcher.register("img:save:success", function (data) {
    currPic = data;
    _this7.emit("img:save:success");
  });

  this.Dispatcher.register("img:save:failed", function (data) {
    _this7.emit("img:save:failed");
  });
